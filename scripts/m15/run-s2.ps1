...
function Assert-ReactorShape {
    ...
    if ($modules -notcontains 'minos-app') {
        throw "M15-S2 reactor must contain minos-app; modules=$($modules -join ',')."
    }
...
try {
    ...
    Write-Host '=== MINOS M15-S2 - reactor update + exact-head validation ===' -ForegroundColor Cyan
