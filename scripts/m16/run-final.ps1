...
function Restart-UpdatedRunner {
    ...
    $args = @('-NoProfile','-ExecutionPolicy','Bypass','-File',(Join-Path $RepoRoot 'scripts\m16\run-final.ps1'),'-Repetitions',[string]$Repetitions)
...
function Assert-Structure {
    ...
    foreach ($relative in $required) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Required M16 file is missing: $relative" }
    }
    ...
    if ($datasets.schemaVersion -ne 1 -or $datasets.seed -ne 16000031) { throw 'Unexpected M16 dataset manifest version/seed.' }
    ...
    if ($mainSources.Count -ne 208) { throw "Unexpected M16 production source count: expected=208 actual=$($mainSources.Count)" }
    if ($testSources.Count -ne 100) { throw "Unexpected M16 test source count: expected=100 actual=$($testSources.Count)" }
}
...
try {
    ...
    if ($dirty.Count -gt 0) { throw "M16 final runner requires a clean worktree.`n$($dirty -join "`n")" }
    $initialHead = ((& git rev-parse HEAD) | Select-Object -First 1).Trim()

    Write-Host '[1/9] Fetching/finalizing M16 branch...'
    ...
    Write-Host '[2/9] Checking M16 structural/data/backend boundaries...'
    Assert-Structure
    $python = Resolve-Python
    Invoke-PythonChecked -Python $python -Arguments @('scripts/docs/product-facts.py','--check') -Failure 'Product facts consistency failed'

    Write-Host '[3/9] Replaying clean verify + complete M14/providers/Windows qualification...'
    ...
    if ([long]$baseline.junit.tests -ne 260 -or [long]$baseline.junit.failures -ne 0 -or [long]$baseline.junit.errors -ne 0) {
    ...
    if ([long]$baseline.mainSourceCount -ne 208 -or [long]$baseline.testSourceCount -ne 100 -or [long]$baseline.reactorModules -ne 13) {
    ...
    Invoke-PythonChecked -Python $python -Arguments @('scripts/quality/check-jacoco.py') -Failure 'JaCoCo targeted gates failed'
    ...
    if (-not (Test-Path -LiteralPath $installedJar -PathType Leaf)) { throw "M14 installed JAR is missing: $installedJar" }
    if (-not (Test-Path -LiteralPath $validationHome -PathType Container)) { throw "M14 validation home is missing: $validationHome" }
    ...
    Write-Host '[4/9] Running STANDARD scale/query/memory/disk benchmark...'
    ...
    Write-Host '[5/9] Running sustained MCP benchmark on one long-lived server...'
    ...
    Write-Host '[6/9] Running real-provider FULL/NONE indexing benchmark...'
    & (Join-Path $RepoRoot 'scripts\m16\run-indexing-benchmark.ps1') `
        -JarPath $installedJar -ValidationHome $validationHome -OutputJson (Join-Path $resultRoot 'indexing.json')

    Write-Host '[7/9] Comparing experimental SQLite access paths and applying backend decision rule...'
    Invoke-PythonChecked -Python $python -Arguments @(
        'scripts/m16/sqlite-backend-probe.py','--profile','STANDARD','--repetitions',[string]$Repetitions,
        '--database',(Join-Path $resultRoot 'sqlite.db'),'--output',(Join-Path $resultRoot 'sqlite.json')
    ...
    Write-Host '[8/9] Verifying snapshot/run retention and final metric gates...'
    Invoke-NativeChecked -File $java.JavaExecutable -Arguments @(
        '--class-path',$installedJar,
        (Join-Path $RepoRoot 'scripts\m16\M16RetentionProbe.java'),
        (Join-Path $m16Temp 'retention'),(Join-Path $resultRoot 'retention.json')
    ...
    Write-Host '[9/9] Verifying exact HEAD did not move...'
    $finalHead = ((& git rev-parse HEAD) | Select-Object -First 1).Trim()
    if ($finalHead -ne $head) { throw "HEAD moved during M16 qualification: start=$head final=$finalHead" }
    $finalDirty = @(& git status --porcelain)
    if ($finalDirty.Count -gt 0) { throw "M16 qualification dirtied the worktree.`n$($finalDirty -join "`n")" }
