...
public record DistributedArtifactCachePolicy(
    ...
    public DistributedArtifactCachePolicy {
        ...
        if (maxArtifactBytes < 1 || maxArtifactBytes > IndexArtifactLimits.MAX_SCIP_ARTIFACT_BYTES) {
            throw new IllegalArgumentException("maxArtifactBytes must stay within the authoritative SCIP artifact limit");
        }
