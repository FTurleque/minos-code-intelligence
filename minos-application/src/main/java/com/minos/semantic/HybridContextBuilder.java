...
public final class HybridContextBuilder {
    ...
    public ContextResponse build(String projectReference, ContextRequest request) throws IOException {
        ...
        for (HybridSearchService.HybridHit hit : ranked.hits()) {
            if (selected.size() >= request.maxDocuments()) {
                truncated = true;
                break;
            }
            ...
            selected.add(new ContextItem(
                    hit.document().stableKey(),
                    hit.document().kind().name(),
                    hit.document().sourceId(),
                    hit.document().fileId(),
                    hit.document().startLine(),
                    hit.document().endLine(),
                    content,
                    hit.score(),
                    hit.rankingMode(),
                    hit.signals(),
    ...
    public record ContextRequest(
        ...
        public static ContextRequest defaults(String query) {
            return new ContextRequest(query, 10, 4_000, 800);
        }
    }
    ...
    public record ContextItem(
            ...
            double rankingScore,
            String rankingMode,
            List<HybridSearchService.RankingSignal> signals,
            int estimatedTokens,
            boolean truncated
    ) {
        public ContextItem {
