...
public final class HybridSearchService {
    ...
    public HybridResponse search(String projectReference, HybridRequest request) throws IOException {
        ...
        for (SemanticDocument document : corpus.documents()) {
            ...
            double graph = document.kind() == SemanticDocumentKind.SYMBOL && corpus.maxDegree() > 0
                    ? corpus.graphDegree().getOrDefault(document.sourceId(), 0) / (double) corpus.maxDegree()
                    : 0.0;
    ...
    private CachedCorpus corpus(
        ...
        List<SemanticDocument> documents = indexedDocuments != null
                ? List.copyOf(indexedDocuments)
                : documentFactory.build(project, snapshot);
        ...
        corpusCache.put(project.id(), next);
        return next;
    }

    private Map<String, Double> semanticScores(
    ...
    static double lexicalScore(String query, String content) {
        return LexicalQuery.compile(query).score(content);
    }
    ...
    private static String normalize(String value) {
        ...
        for (int offset = 0; offset < lower.length();) {
            ...
            if (isTermCodePoint(codePoint)) {
                normalized.appendCodePoint(codePoint);
                separating = false;
            } else if (!separating && !normalized.isEmpty()) {
                normalized.append(' ');
                separating = true;
            }
        }
    ...
    private record CachedCorpus(
            ...
            Map<String, Integer> graphDegree,
            int maxDegree
    ) {
        CachedCorpus {
