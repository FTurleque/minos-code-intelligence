...
public final class ProjectIgnorePolicy {
    ...
    private ProjectIgnorePolicy(Path root, SourceBudgetPolicy.Tracker budget,
                                List<IgnoreRule> gitRules, List<IgnoreRule> minosRules) {
        this.root = root;
        this.budget = budget;
    ...
    public static ProjectIgnorePolicy load(Path projectRoot) throws IOException { return load(projectRoot, null); }

    static ProjectIgnorePolicy load(Path projectRoot, SourceBudgetPolicy.Tracker budget) throws IOException {
        Objects.requireNonNull(projectRoot, "projectRoot");
        Path root = projectRoot.toAbsolutePath().normalize();
        return new ProjectIgnorePolicy(root, budget,
                readRules(root.resolve(".gitignore")), readRules(root.resolve(".minosignore")));
    }
    ...
    private static boolean hardIgnored(Path normalized) {
        for (Path segment : normalized) if (HARD_IGNORED_DIRECTORY_NAMES.contains(segment.toString())) return true;
        return false;
    }

    private void accountTraversal() {
        if (budget == null) return;
        try { budget.accountTraversalEntry(); }
    ...
    private static boolean evaluate(List<IgnoreRule> rules, String portablePath, boolean directory) {
        ...
        for (IgnoreRule rule : rules) if (rule.matches(portablePath, directory)) ignored = !rule.negated();
        return ignored;
    }
    ...
    private static IgnoreRule parseRule(String rawLine) {
        ...
        Pattern effectivePattern = directoryOnly
                ? Pattern.compile(baseExpression + "(?:/.*)?$") : directPattern;
    ...
    private static String globToRegex(String glob) {
        ...
        while (index < glob.length()) {
            ...
            if (current == '*') {
                ...
                } else { regex.append("[^/]*"); index++; }
                continue;
            }
            if (current == '?') { regex.append("[^/]"); index++; continue; }
            if (current == '[') {
                int closing = glob.indexOf(']', index + 1);
                if (closing > index + 1) {
                    String characterClass = glob.substring(index + 1, closing);
                    regex.append('[');
                    if (characterClass.startsWith("!")) { regex.append('^'); characterClass = characterClass.substring(1); }
    ...
    private static Path normalizeRelative(Path path) {
        ...
        if (normalized.startsWith("..")) throw new IllegalArgumentException("relativePath must stay inside the project root");
        return normalized;
    }

    private static String portable(Path path) { return path.toString().replace('\\', '/'); }

    private record IgnoreRule(Pattern effectivePattern, Pattern directPattern,
                              boolean negated, boolean directoryOnly) {
