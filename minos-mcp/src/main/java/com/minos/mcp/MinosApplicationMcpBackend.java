...
final class MinosApplicationMcpBackend implements MinosMcpBackend {
    ...
    public String searchCode(SearchRequest request) throws Exception {
        ...
        return CodeSearchRenderer.render(
                queries.searchCode(request.project(), criteria),
    ...
    public String findUsages(RelationRequest request) throws Exception {
        return CodeIntelligenceResultRenderer.renderUsages(
                queries.findUsages(request.project(), request.symbolId(), request.limit()),
                SymbolOutputFormat.JSON
        );
    }
    ...
    public String findRelationships(RelationshipOperation operation, RelationRequest request) throws Exception {
        ...
        switch (operation) {
            ...
            case RELATED_TESTS -> {
                kind = RelationshipKind.RELATED_TEST;
                incoming = true;
            }
    ...
    public String symbolContext(SymbolContextRequest request) throws Exception {
        return searchCode(new SearchRequest(
                ...
                1,
                request.depth(),
                DEFAULT_SEARCH_USAGES,
                DEFAULT_SEARCH_RELATIONSHIPS,
                request.contextLines(),
                request.maxTokens(),
                request.includeSource()
        ));
    ...
    public String moduleContext(String project, String module) throws Exception {
        return ArchitectureResultRenderer.renderModule(
                application.architectureQuery().getModuleContext(project, module),
                SymbolOutputFormat.JSON
        );
    }
    ...
    public String architecture(String project) throws Exception {
        return ArchitectureResultRenderer.render(
                application.architectureQuery().getArchitectureIntelligence(project),
                SymbolOutputFormat.JSON
        );
    }
    ...
    public String architectureGraph(ArchitectureGraphRequest request) throws Exception {
        ...
        if ("json".equals(format)) {
            return request.module() == null
                    ? architecture(request.project())
                    : moduleContext(request.project(), request.module());
        }

        ArchitectureIntelligenceView view = application.architectureQuery()
                .getArchitectureIntelligence(request.project());
        String moduleId = request.module() == null
                ? null
                : application.architectureQuery()
                        .getModuleContext(request.project(), request.module())
                        .module().id();
    ...
    public String impact(ImpactRequest request) throws Exception {
        return ImpactResultRenderer.render(
                application.impactQuery().analyzeImpact(
                        request.project(),
                        new ImpactAnalysisRequest(request.symbolId(), request.depth(), request.limit())),
                SymbolOutputFormat.JSON
        );
    }

    private static SymbolSearchCriteria symbolCriteria(
            String query,
        ...
        return new SymbolSearchCriteria(
                query,
                qualifiedName,
                parseKind(kind),
                module,
