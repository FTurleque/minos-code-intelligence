...
| R-08 | PostgreSQL backend non promu | Résolu par M30 et durci post-audit : bootstrap (extension pgvector, création de schéma) et migrations sérialisés dès la première opération sous le même advisory lock global, schéma v2, unicité racine et upsert atomique. |
