...
- **DT-10 — test Windows racy.** Observation du PID enfant rendue déterministe (publication atomique, lecture après observation, exécution du scénario de timeout sur son propre thread). Aucun sleep d'attente, aucun retry, aucun skip, aucune assertion de sécurité modifiée.
