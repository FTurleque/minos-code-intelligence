    ...
    public static int Run(string[] command, string[] environment, string workingDirectory) {
        ...
        try {
            ...
            if (!CreateProcessW(
                    ...
                    environmentBlock, workingDirectory, ref startup, out pi)) {
            ...
            if (!inJob) throw new InvalidOperationException("MINOS CLI is not a member of the Job Object");
    ...
    private static void ConfigureAndVerifyJob(IntPtr job) {
        ...
        try {
            ...
            if ((flags & JOB_OBJECT_LIMIT_BREAKAWAY_OK) != 0
                ...
                throw new InvalidOperationException("MINOS CLI Job Object must not allow breakaway");
            }
        } finally {
...
function Read-Environment($Values) {
    ...
    for ($index = 0; $index -lt $count; $index++) {
        ...
        if ([string]::IsNullOrEmpty($key) -or $key.IndexOf([char]0) -ge 0 -or $value.IndexOf([char]0) -ge 0) {
            throw 'Invalid CLI environment entry in ownership plan'
        }
